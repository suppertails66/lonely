...
void Graphic::resize(int w__, int h__) {
  // If dimensions already match, do nothing
  if ((w_ == w__) && (h_ == h__)) {
    return;
  }
  
  *this = Graphic(w__, h__);
  return;
}
...
Tcolor Graphic::getPixel(int xpos, int ypos) {
...
void Graphic::scaleOld(Graphic& dst,
                    Box dstbox,
                    TransBlitOption updateTrans) {
...
void Graphic::scale(Graphic& src,
                    Box dstbox,
                    TransBlitOption updateTrans) {
  src.scaleOld(*this,
               dstbox,
...
void Graphic::scaleAndCopy(Graphic& src,
                  Box dstbox,
                  TransBlitOption updateTrans) {
  // If dimensions match, no need to scale
  if ((src.w() == dstbox.w()) && src.h() == dstbox.h()) {
...
void Graphic::clear(Tcolor color) {
  // Do "slow" clear.
